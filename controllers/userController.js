...
exports.getAllUsersCms = catchAsync(async (req, res, next) => {
  ...
  console.log(docs);
  const total = await User.find({ isAdmin: false }).countDocuments();
  res.status(200).json({
    status: "success",
    data: {
      docs,
...
exports.getAllAdminsCms = catchAsync(async (req, res, next) => {
  const features = new ApiFeatures(User.find(), req.query)
    .pagination()
    .sort()
    .filter();
  const docs = await features.query;
  const total = await User.find({ isAdmin: true }).countDocuments();
  res.status(200).json({
    status: "success",
    data: {
      docs,
